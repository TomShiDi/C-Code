...
//程序开始执行，argc=1，执行Parent方法，创建一个hMutexSuicide互斥体，如果创建成功，执行StartClone方法创建子进程。在StartClone方法中，GetModuleFileName方法获取
//了父进程的完整路径，然后在路径后添加了child参数，即子进程的main方法传入的参数有两个，argc=2，ZeroMemory方法将子进程占用的内存置0，CreateProcess方法创建子进程。
//Parent方法执行完StartClone方法后遇到getchar方法而阻塞，同时父进程持有互斥锁hMutexSuicide，子进程调用了Child方法，打开互斥体，但此时互斥锁被父进程持有，
//WaitForSingleObject方法阻塞子进程的继续执行直到获得互斥锁,
//当父进程的终端有字符输入，getchar方法阻塞解除，释放互斥锁，子进程获得互斥锁，子进程可以执行后面的代码直到结束。